...
function init(ctx) {
  ...
  loop.updateVisualisation = function updateVisualisation (sbx) {
	...
	var eventualBGscaled = sbx.settings.units === 'mmol' ? Math.round(eventualBG/1.802)/10 : eventualBG;
        var maxBGscaled = sbx.settings.units === 'mmol' ? Math.round(maxBG/1.802)/10 : maxBG;
        var minBGscaled = sbx.settings.units === 'mmol' ? Math.round(minBG/1.802)/10 : minBG;
        valueParts = valueParts.concat([
