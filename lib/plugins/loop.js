...
function init(ctx) {
  ...
  loop.updateVisualisation = function updateVisualisation (sbx) {
	...
	var eventualBGscaled = sbx.roundBGToDisplayFormat(sbx.scaleMgdl(eventualBG));
        var maxBGscaled = sbx.roundBGToDisplayFormat(sbx.scaleMgdl(maxBG));
        var minBGscaled = sbx.roundBGToDisplayFormat(sbx.scaleMgdl(minBG));
        valueParts = valueParts.concat([
